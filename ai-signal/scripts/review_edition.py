...
def render_index(data):
    ...
    ready_command = f"python3 ai-signal/scripts/review_edition.py --source ai-signal/content/review/{edition_id}.json --output ai-signal/.review/{edition_id} --ready"
    body = f'''<p class="section-kicker">Edition {e(edition_id)} / Prepared {e(data["edition"]["preparedAt"])}</p><h1>{e(data["edition"]["title"])}</h1><div class="demonstration"><strong>Review bundle.</strong> {e(data["edition"]["note"])}</div><section><h2>Story review queue</h2><div class="stories">{cards}</div></section><section><h2>Proposed Signal Map changes</h2><p>Every proposal below resolves to a story in this review edition. Promotion remains blocked until all stories are approved.</p><ul>{moves}</ul></section><section><h2>Approval procedure</h2><ol><li>Open each story and verify every fact against its linked primary source.</li><li>Review rights, fairness, uncertainty, headline, analysis, and tone.</li><li>Set the four checklist fields to true, status to approved, and record reviewer and reviewedAt.</li><li>Set edition.publicationDate.</li><li>Run the ready check and promotion preview before changing the live edition.</li></ol><pre>{e(ready_command)}</pre></section>'''
    return page_shell(f"{edition_id} review", body, 1, "index.html")
